...
Some objects are movable and others immovable, similarly note every object may be copied.

C++ has both move assignment and copy assignment as well as copy and move constructors.

To improve the ability to reason about code, there should only be move assignment
and copy constructors. Copy assignment for primitive value types should be explicitly
declared by a combination of copy constructor and move assignment.

Since we do not assign, but initialize.

Objects may be copyable, movable, neither, or both.

- `moveable` - The object does not depend on references to its location. (no mutexes, no cyclic pointers, no non-movable members, etc.)
...
Type expressions containing references or pointers may indicate lifetime constraints for objects passed as arguments to methods, functions, or constructors.
