...
includes hardware changing memory state, co-processors, or interrupt contexts.

One common mistake is assuming that the cv-qualifier `volatile` is sufficient
to provide unoptimized and atomic access to shared memory.

This is not correct. Volatile objects are accessed in order and verbatim within
program execution, but that does not guarantee atomicity, and does not prevent
