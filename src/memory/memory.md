...
The smallest unit of addressable memory is teh byte. (exactly 8 bits)
...
When a reference is constant and the referee is also constant, the reference may be optimized out entirely.
...
Unions are overlapping storage with the alignment and size of the greatest member.
...
Variants are structures which contain an enum and an ID member identifying the active member.
...
arrays may be dynamically sized. (But not in coroutines)
